 ...
 *   ✨ Architecture v2  — High-fidelity HTML diagrams
...
import { VoiceLiveAPIArchitectureV2 } from './ArchitectureDiagramsV2';
...
const VLAChatBot: React.FC = () => {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (isLocalhost) {
        // Local development: connect to local server on port 3001
        const protocol = window.location.protocol === 'https:' ? 'wss' : 'ws';
        wsUrl = `${protocol}://${window.location.hostname}:3001/api/voicelive/ws`;
      } else {
        // Production: connect to Azure App Service backend
        wsUrl = 'wss://thr505-api-server.azurewebsites.net/api/voicelive/ws';
      }
      
      console.log('🎙️ [VLA] Connecting to:', wsUrl);
...
export const VoiceLiveAPI: React.FC = () => {
  ...
  return (
    <div className="chat-container vla-container">
      ...
      {/* ── Architecture Tab ────────────────────────────────────────────────── */}
      {activeTab === 'architecture' && <VoiceLiveAPIArchitectureV2 />}

      {/* Old inline architecture kept as comment reference */}
      {false && (
